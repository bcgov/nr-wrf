...
export class ZipFileService {
   ...
   */
  /**
   * Builds the working-directory path for a job.
   *
   * subFolder is always a UUID generated by this service, but it reaches the
   * service as a raw request parameter, so it is stripped to a bare filename
   * and checked against the UUID format before being joined to the base path.
   * Without this, a value such as "../../etc" would escape the intended
   * directory (CodeQL js/path-injection, "Uncontrolled data used in path
   * expression").
   */
  private static readonly UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

  private resolveJobFolder(subFolder: string): string {
    const safeName = path.basename(String(subFolder));
    if (!ZipFileService.UUID_PATTERN.test(safeName)) {
      throw new BadRequestException('Invalid job identifier');
    }
    return path.join(process.env.filePath, safeName) + '/';
  }

  checkZipFile(subFolder: string): { status: string; num: string } {
    const folder = this.resolveJobFolder(subFolder);
