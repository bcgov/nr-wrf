 ...
 * per WRF domain.  Any service that needs to convert lat/lon → i/j simply
...
export class DomainProjectionConfig implements OnModuleInit {
  ...
  private loadCsv(): void {
    ...
    if (!fs.existsSync(csvPath)) {
      throw new Error(
        `DomainProjectionConfig: file not found → ${csvPath}\n` +
        `Please place domain_projection.csv in src/util/.`,
      );
    }
    ...
    // Build a column-index lookup so CSV column order doesn't matter
    const col = (name: string): number => {
      const idx = headers.indexOf(name);
      if (idx === -1) {
        throw new Error(`domain_projection.csv: required column "${name}" is missing.`);
      }
      return idx;
    };
    ...
    for (let n = 1; n < lines.length; n++) {
      ...
      this.logger.log(
        `Loaded projection for ${domain}: ` +
        `lat1=${proj.lat1}, lon1=${proj.lon1}, dx=${proj.dx} m`,
      );
    }

    this.logger.log(`DomainProjectionConfig: ${this.projections.size} domain(s) loaded.`);
  }
   ...
   * Throws a clear error if the domain name is not in the CSV.
   */
